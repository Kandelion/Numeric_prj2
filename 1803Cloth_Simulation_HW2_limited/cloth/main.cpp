...
void InitializeBuffers() {
    ...
    for (int i = 0; i < NUM_PARTICLES_Y; i++) {
        for (int j = 0; j < NUM_PARTICLES_X; j++) {
			...
			position[pos_idx++] = p.z;
			position[pos_idx++] = 1.0f;
#endif
        }
    }
...
void calcSpringForce(int x, int y, const GLfloat *pos, GLfloat *result) {
	...
	float rh = REST_LENGTH_HORIZ;
	float rv = REST_LENGTH_VERT;
	float rd = REST_LENGTH_DIAG;

	idx1 = 4 * (y*NUM_PARTICLES_X + x);
	...
	for (int i = y - 1; i <= y + 1; i++) {
		for (int j = x - 1; j <= x + 1; j++) {
			...
			idx0 = 4 * (i*NUM_PARTICLES_X + j);
			r[0] = pos[idx0] - pos[idx1];
			r[1] = pos[idx0 + 1] - pos[idx1 + 1];
			r[2] = pos[idx0 + 2] - pos[idx1 + 2];
			distance = sqrt(r[0] * r[0] + r[1] * r[1] + r[2] * r[2]);

			if (i == y) {
				result[0] += SPRING_K * (distance - rh) * (r[0] / distance);
				result[1] += SPRING_K * (distance - rh) * (r[1] / distance);
				result[2] += SPRING_K * (distance - rh) * (r[2] / distance);
			}
			else if (j == x) {
				result[0] += SPRING_K * (distance - rv) * (r[0] / distance);
				result[1] += SPRING_K * (distance - rv) * (r[1] / distance);
				result[2] += SPRING_K * (distance - rv) * (r[2] / distance);
			}
			else {
				result[0] += SPRING_K * (distance - rd) * (r[0] / distance);
				result[1] += SPRING_K * (distance - rd) * (r[1] / distance);
				result[2] += SPRING_K * (distance - rd) * (r[2] / distance);
			}
		}
	}
}

inline void calcGravityForce(GLfloat *result) {
	result[1] += PARTICLE_MASS * GRAVITY[1];
}

void calcDampingForce(int x, int y, const GLfloat *vel, GLfloat *result) {
	result[0] -= DAMPING_CONST * vel[4 * (x + NUM_PARTICLES_X * y)];
	result[1] -= DAMPING_CONST * vel[4 * (x + NUM_PARTICLES_X * y) + 1];
	result[2] -= DAMPING_CONST * vel[4 * (x + NUM_PARTICLES_X * y) + 2];
}

void CalcPosition(GLfloat *pos, GLfloat *vel) {
	int pos_cnt = 0;
	float F[3] = { 0 };
